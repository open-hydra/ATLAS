! Map unwrapped line-probe solutions onto 3D cylindrical block faces.
! The flat line is wrapped onto the cylinder circumference.
! Periodic interpolation in theta-space with velocity transformation.
module unwrapped_to_3D_mod
  ...
  use read_mesh_mod, only: read_mesh
  use Lib_ORION_data
  use Lib_Tecplot
  implicit none
  private
...
contains
  ...
  subroutine map_unwrapped_to_3D(linefile, outfile, face, strip_j_face, cyl_center)
    ...
    ! Read line probe file (Tecplot structured multiblock): auto-detect circ coord, read variables and times
    if (verbose) write(*,'(A,A)') " [LOG] Reading line-file: ", trim(linefile)
  ...
  end subroutine map_unwrapped_to_3D


  !----------------------------------------
  ! Extract strip from a zone: auto-detect circ coord, convert to theta, copy variables
  ...
  subroutine interpolate_vars(centers, vars, strip, ax_idx, center, n1, n2)
    ...
    ! Map unwrapped vel -> tangential/axial
  ...
  subroutine extract_geometry(blk, face, n1, n2, nodes, centers)
    ...
    do i2 = 1, n2
      do i1 = 1, n1
        centers(:,i1,i2) = blk%face(face)%center(i1,i2)%c(1:3)
      enddo
    enddo
    !$omp end parallel do
  end subroutine
...
end module unwrapped_to_3D_mod