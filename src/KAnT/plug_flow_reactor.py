...
for model in models:
    ...
    oxidizer_gas.TPY = 3500, 140*ct.one_atm, 'O2:0.013665853353367602, H2O:0.424505918181108666, CO:0.267129606823931831, CO2:0.241378707163126927, H2:0.00984026955481566781, H:0.000925043960154194825, O:0.00377335330710388281, OH:0.0387812285889810038'

    mdot = 1000000000000000 # kg/(m2 s)
    ...
    while t<time:
        ...
        o2_profile.append(fuel_gas['O2'].Y[0])
        ch4_profile.append(fuel_gas['CH4'].Y[0])
        species_profile.append(fuel_gas[species_name].Y[0])
...
plt.xlabel('Distance (m)')
plt.ylabel(f'Mass Fraction of {species_name}')
plt.grid(True)
plt.legend()
