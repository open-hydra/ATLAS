...
contains
  ...
  subroutine area_variation_for_block(block, sini, section_name, block_id)
    ...
    do idir = 1, NDIR
      call sini%get(section_name=section_name, &
                    option_name=trim(DIR_KEYS(idir)), &
    ...
    coord = get_direction_coord(block%node(0:ni,0:nj,0)%c(1), &
                      block%node(0:ni,0:nj,0)%c(2), DIR_KEYS(idir))
