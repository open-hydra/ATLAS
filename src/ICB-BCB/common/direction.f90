...
contains
  ...
  subroutine parse_direction(dirID, dir, ndir, index_based, injection_plate)
    character(*),              intent(in)            :: dirID
    integer,      allocatable, intent(out)           :: dir(:)
    integer,                   intent(out)           :: ndir
    logical,                   intent(out)           :: index_based
    logical,                   intent(out), optional :: injection_plate
    ! Local
    logical :: found(8)
    integer :: i
    ...
    do i = 1, ndir
      ...
      if (index(dirID,'plate')/=0) injection_plate=.true.
