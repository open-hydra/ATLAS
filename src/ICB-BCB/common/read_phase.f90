module io_phase_mod
  ...
  public :: read_realfluid_properties
  public :: read_dp_properties
  public :: read_phase
...
contains
  ...
  subroutine read_idealgas_properties(prefix, sp)
    ...
    integer           :: ios, i, n, unitfile, start, Ti1, Ti2, dummy1, dummy23
  ...
  subroutine read_realfluid_properties(prefix, sp)
    use phase_mod, only: real_fluid_t
    use strings, only: parse
    use Lib_Tecplot
    implicit none
    character(len=*), intent(in):: prefix
    type(real_fluid_t), intent(inout) :: sp
    ...
    read(unitFile,'(A)') wholestring
    call parse(wholestring,' ',args)
    sp%name = trim(adjustl(args(1)))
    close(unitFile)

    !! File 2: thermo — use ORION to get mesh (p, h node coords), read T directly
    ios = tec_read_structured_multiblock(orion=orion,filename=trim(prefix)//'thermo.dat')
    if (ios/=0) then
      write(*,*) '[WARNING] thermo file, '//trim(prefix)//'thermo.dat'//' not found'
      return
    endif

    ! sp%h(ip): I-axis → mesh(2, i=0..Ni, j=0, 0)
    ! sp%p(ih): J-axis → mesh(1, i=0, j=0..Nj, 0)
    ! sp%T(ip, ih): Temperature at (pressure index, enthalpy index)
    allocate(sp%p(0:orion%block(1)%Ni))
    allocate(sp%h(0:orion%block(1)%Nj))
    allocate(sp%T(0:orion%block(1)%Ni, 0:orion%block(1)%Nj))
    sp%p = orion%block(1)%mesh(1, :, 0, 0)
    sp%h = orion%block(1)%mesh(2, 0, :, 0)

    do j = 0, orion%block(1)%Nj
      do i = 0, orion%block(1)%Ni
        sp%T(i, j) = orion%block(1)%vars(2, i, j, 0)
      enddo
    enddo

  end subroutine read_realfluid_properties
  ...
  end subroutine read_dp_properties
