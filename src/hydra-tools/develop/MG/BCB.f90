...
program BCB
  ...
  use bc_area_variation_mod
  use finer, only: file_ini
  implicit none
  type(phase_t), allocatable  :: phase(:)
  type(BC_block), allocatable :: block(:)
  ...
  do m = 1, MG_levels
    ...
    if ( m == 1 ) then
      allocate(block(size(fine_orion%block)))
