...
contains
  ...
  subroutine run_q2d_bc_map(meshfile, q2dfile, outfile, face, strip_j_face, cyl_center)
    ...
    type(BC_block), allocatable :: blocks(:)
    ...
    allocate(blocks(size(mesh_orion%block)))
    call import_nodes(input=mesh_orion, output=blocks)
    do b = 1, size(blocks)
      call blocks(b)%build_geometry()
      call blocks(b)%compute_face_centers()
    enddo
